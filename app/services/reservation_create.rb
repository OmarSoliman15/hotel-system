  ...
  def validate_reservation
    ...
    params[:room_types].each do |room_type|
      ...
      if room_type_obj.present?
        reservation.errors.add(:base, "Room(s) availability is not matched") if room_type_obj.available_rooms < room_type["number_of_rooms"]

        reservation.errors.add(:base, "Room type(s) not available in this range") if room_type_not_available?(room_type_obj)
